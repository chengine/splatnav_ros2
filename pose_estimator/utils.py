...
def execute_PnP_RANSAC(nerf, init_guess,
                       rgb_input,
                       camera_intrinsics_K,
        ...
        target_rgb, target_pcd_pts, _, depth_mask, *_ = nerf.generate_RGBD_point_cloud(init_guess,
        ...
        # import matplotlib.pyplot as plt
        # img = (target_rgb.cpu().numpy() * 255).astype(np.uint8)
        ...
        if feature_detector == POI_Detector.LIGHTGLUE:
            ...
            # match the features
            matches = detector_params['matcher']({'image0': source_feats, 'image1': target_feats})
...
# function
def compute_sigmapoints(state_mu: torch.Tensor, state_cov: torch.Tensor, kappa: float = 2.0, device:torch.device = 'cuda'): 
    # state dimension
...
def dynamics_step(state: torch.Tensor, 
...
def inverse_sigma_transform_prediction(state_mu_samples: torch.Tensor, kappa: float = 2.0,
...
def inverse_sigma_transform_measurement(mu_x_diff: torch.Tensor, pred_x_mean: torch.Tensor,
                                        meas_mu_samples: torch.Tensor, kappa: float = 2.0,
    ...
    # weights
    weights = torch.concat((torch.tensor([kappa / (meas_dim + kappa)]),
                              torch.broadcast_to(torch.tensor([1 / (2 * (meas_dim + kappa))]),
                                                 (meas_mu_samples.shape[0] - 1,)))).to(device)
...
# def se_3_to_SE_3(pose_se_3: torch.Tensor,
#                  device:torch.device = 'cuda'):
#     # map from se(3), the lie algebra, to SE(3)
#     pose_SE_3 = torch.linalg.matrix_exp(torch.concat((torch.concat((skew_symmetric(pose_se_3[3:6]),
#                                                                     pose_se_3[:3][:, None]),
#                                                                    dim=-1),
#                                                     torch.zeros(4, device=device)[None]),
#                                                     dim=-2))
...
#     pose_SE_3_log = torch.tensor(sci_logm(pose_SE_3.cpu()), device=device)
    
#     # pose in se(3)
#     pose_se_3 = torch.empty(6, device=device)
    
#     # translation components
#     pose_se_3[:3] = pose_SE_3_log[:3, -1]
    
#     # rotation components
#     pose_se_3[3] = pose_SE_3_log[2, 1]
#     pose_se_3[4] = pose_SE_3_log[0, 2]
#     pose_se_3[5] = pose_SE_3_log[1, 0]
    
#     return pose_se_3

# def measurement_innovation(y: torch.Tensor,
#                            y_mean: torch.Tensor,
#                            device:torch.device = 'cuda'):
#     # map from se(3), the lie algebra, to SE(3)
#     exp_meas = torch.linalg.matrix_exp(torch.concat((torch.concat((skew_symmetric(y_mean[3:]),
#                                                                    y_mean[:3][:, None]), 
#                                                                   dim=1),
#                                                     torch.zeros(4, device=device)[None]),
#                                                     dim=0))
    
#     # geodesic error (rotation)
#     geod_error = y[:3, :3].T @ exp_meas[:3, :3]
    
#     # translation error
#     trans_error = y[:3, -1] - exp_meas[:3, -1]
    
#     # map from SE(3) to se(3)
#     innovation = torch.eye(4, device=device)
#     innovation[:3, :3] = geod_error
#     innovation[:3, -1] = trans_error
    
#     return SE_3_to_se_3(innovation)

# def measurement_innovation(y: torch.Tensor,
#                            y_mean: torch.Tensor,
#                            device:torch.device = 'cuda'):
#     # map from se(3), the lie algebra, to SE(3)
#     y_mean_SE3 = se3_to_SE3(y_mean)

#     # geodesic error (rotation)
#     geod_error = y[:3, :3].T @ y_mean_SE3[:3, :3]
    
#     # translation error
#     trans_error = y[:3, -1] - y_mean_SE3[:3, -1]
...
def correction_phase_post_measurement(y: torch.Tensor,
                                      ...
                                      device:torch.device = 'cuda'):
    # Kalman gain
    K_gain = xy_cov @ torch.linalg.solve(yy_cov, torch.eye(yy_cov.shape[0], device=device))
